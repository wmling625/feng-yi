...
if (!empty($cmd) && !empty($value) && !empty($token)) {
    ...
    if (token_validation($value, $token)) {
        ...
        // QRCode紀錄
        $title = [['建立時間', '單位名稱', '會員姓名', '會員手機', '會員性別', '活動區域', '綁定狀態', '綁定時間']];
...
//                }
                ...
                $arrData[] = [$row["pub_date"], $row["title"], $row["nickname"], $row["account"], $row['types_option'], $row["city"] . $row["region"] , $isBind, $row["bind_date"]];
            }
...
//        $spreadsheet->getActiveSheet()->getColumnDimension('Z')->setWidth(25);
        ...
        header("Content-Disposition: attachment;filename=\"QRCode綁定列表_" . date("Ymdhis") . ".xlsx\"");
