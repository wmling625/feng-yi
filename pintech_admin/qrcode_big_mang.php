...
@$qrcode_big_id = params_security($_GET["qrcode_big_id"]);
...
$box_arr2 = array();
$query = "SELECT `qrcode`.*, `member`.`title` AS name, `member`.`account`, 1 AS qrcode_big FROM qrcode_big LEFT JOIN `member` ON `qrcode`.`member_id`=`member`.`member_id` WHERE qrcode_big_id = '" . $qrcode_big_id . "';";
        ...
        if ($result = $mysqli->store_result()) {
            while ($row = $result->fetch_assoc()) {
                ...
                if (isset($row['qr_type_big'])) {
                    $box_arr[$row["title"]] = $row["qr_type_big_id"];;
                }
                if (isset($row['member'])) {
                        ...
                        <form method="post" id="form" enctype="multipart/form-data"
                            ...
                            <div class="card">
                                <div class="card-body table-responsive-sm">
                                    <table class="table table-bordered">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select req data-title="廣告標籤" name="qr_type_big_id"
                                                        class="form-control"
                                                        defaults="<?php echo isset($result_arr[0]["qr_type_big_id"]) ? $result_arr[0]["qr_type_big_id"] : $qr_type_big_id; ?>">
                                ...
                                <div class="card-footer d-flex justify-content-end">
                                    ...
                                    <input type="hidden" name="qrcode_big_id" value="<?php echo $qrcode_big_id; ?>">
