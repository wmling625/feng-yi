...
if (count($err_msg)) {
    ...
    if ($model == "add") {
        $query = "insert into qr_type (`qr_type_id`, `title`, `pub_date`, `last_date`, `orders`) values (uuid(),'" . $title . "',now(),now(),1)";
    } else if ($model == "update") {
        // 可以對照insert欄位, 略過pub_update..等
        $query = "update qr_type set title = '" . $title . "', last_date = now()  ";
        if (count($file_sql_arr) > 0) {
            $query .= "," . implode(",", $file_sql_arr);
        }
        $query .= "where qr_type_id = '" . $qr_type_id . "';";
    }
