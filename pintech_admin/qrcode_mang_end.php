...
@$qr_type_id = params_security($_POST["qr_type_id"]);
...
if (empty($orders) || empty($qr_type_id) || empty($code)) {
    ...
    if ($model == "add") {
        ...
        if ($number > 0) {
            for ($i = 0; $i < $number; $i++) {
                ...
                /* 產生QRCode圖片 */
                $link = "https://liff.line.me/" . $liff_full . "?end_point=" . aes_encrypt("code_redirect.php?qrcode_id=" . aes_encrypt($uuid));
//                $qrcode = "https://chart.googleapis.com/chart?chs=500x500&cht=qr&chl=" . urlencode($link) . "&choe=UTF-8";
                ...
                $query .= "INSERT INTO qrcode (`qrcode_id`, `member_id`, `code`, `title`, `introd`, `contents`, `file0`, `qr_type_id`, `pub_date`, `last_date`, `is_send`, `orders`) values ('" . $uuid . "','" . $member_id . "','" . $codeRandom . "','" . $title . "','" . $introd . "','" . $contents . "','" . $file0 . "','" . $qr_type_id . "',now(),now(), '" . $is_send . "', '" . $orders . "'); ";
            }
        }
        ...
        $query = "update qrcode set code = '" . $code . "', member_id = '" . $member_id . "', title = '" . $title . "', introd = '" . $introd . "', contents = '" . $contents . "', qr_type_id = '" . $qr_type_id . "', orders = '" . $orders . "', is_send = '" . $is_send . "', last_date = now()  ";
        if (count($file_sql_arr) > 0) {
            $query .= "," . implode(",", $file_sql_arr);
        }
        $query .= "where qrcode_id = '" . $qrcode_id . "';";
