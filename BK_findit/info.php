...
$query = "";
$query .= "SELECT A.*, B.`user_id`, 1 AS 'qrcode' FROM `qrcode` AS A 
            LEFT JOIN `member` AS B ON A.`member_id` = B.`member_id` 
            WHERE A.`qrcode_id` = '" . $qrcode_id . "'; "; // AND A.`orders` >= 0
$query .= "SELECT *, 1 AS 'history' FROM `history` WHERE `qrcode_id`='" . $qrcode_id . "' AND `user_id0` = '" . $user_id0 . "' ORDER BY `pub_date` DESC; ";
...
if (count($result_arr) == 0) {
    ...
    if ($result_arr[0]['orders'] == "-2") {
        ...
        if ($result_arr[0]['user_id'] == "" || $result_arr[0]['orders'] == "-1") {
            echo "<script>alert('此QRCode尚未綁定')</script>"; // 家屬
            echo "<script>document.location.href = 'https://liff.line.me/" . $liff_close . "';</script>";
            exit;
        } elseif ($result_arr[0]['user_id'] == $user_id0) {
            echo "<script>alert('不得留言給自己')</script>";
            echo "<script>document.location.href = 'my_info.php?qrcode_id=" . aes_encrypt($qrcode_id) . "';</script>";
            exit;
        }
    }
}
            ...
            <div class="row justify-content-center no-gutters">
                <div class="col-lg-7">
                    ...
                    <div class="comment-box hidden">
                        <form class="checkthisform" action="info_end.php" method="post"
                            ...
                            <div class="row justify-content-center no-gutters pb-5">
                                <div class="col-lg-8">
                                    ...
                                    <div class="form-item warning-box">
                                        <textarea class="form-control" name="contents0" id="contents0" rows="3"
                                                  placeholder="請輸入留言內容，送出後需間隔5分鐘後才可再次留言"
                                                  req="Y" <?php echo $readonly; ?>><?php echo isset($message_arr[0]['contents0']) ? br2nl($message_arr[0]['contents0']) : br2nl($default_content); ?></textarea>
                                ...
                                <div class="col-8">
                                    ...
                                    <input type="hidden" name="model"
                                           value="<?php echo aes_encrypt($model); ?>">
                                    <input type="hidden" name="qrcode_id"
                                           value="<?php echo aes_encrypt($qrcode_id); ?>">
                                    <input type="hidden" name="qr_type_id"
                                           value="<?php echo aes_encrypt($result_arr[0]['qr_type_id']); ?>">
                                    <input type="hidden" name="user_id0"
                                           value="<?php echo aes_encrypt($user_id0); ?>">
                                    <input type="hidden" name="display_name"
                                           value="<?php echo aes_encrypt($display_name); ?>">
                                    <input type="hidden" name="user_id1"
                                           value="<?php echo aes_encrypt($result_arr[0]['user_id']); ?>">
                                    <input type="hidden" name="history_id"
                                           value="<?php echo aes_encrypt($message_arr[0]['history_id']); ?>">
                                    <input type="hidden" name="license"
                                           value="<?php echo aes_encrypt($result_arr[0]['introd']); ?>">
