...
if (!empty($keyword)) {
    array_push($filter_sql_arr, "(session_content LIKE '%" . $keyword . "%')");
}
...
$query = "SELECT * FROM `pintech_logs`.`listpage` WHERE " . $filter_sql_str . " AND `domains`='" . $domains . "' ORDER BY pub_date DESC";
                                ...
                                <table class="table table-bordered table-hover text-nowrap">
                                    ...
                                    if ($total == 0) {
                                        ...
                                        foreach ($result_arr as $key => $value) {
                                            $arr = logs_replace_json($value['session_content'], 'permission');

                                            echo '<tr>';
                                            echo '<td>' . $value["pub_date"] . '</td>';
                                            echo '<td>' . (isset($arr['admin']['account']) ? $arr['admin']['account'] : "-") . '</td>';
                                            echo '<td class="text-wrap text-left">';
                                            echo '<span class="text-success text-sm">' . ($value["start_link"] !== "" ? str_remove($value["start_link"], 50) . "..." : "-") . '</span><br/>';
                                            echo '<hr class="m-0"><span class="text-secondary text-sm">' . ($value["end_link"] !== "" ? str_remove($_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['SERVER_NAME'] . $value["end_link"], 50) . "..." : "-") . '</span>';
                                            echo '</td>';
                                            echo '<td>' . $value["ip"] . '</td>';
                                            echo '<td>';
                                            echo '<a href="logs_page_mang.php?model=update&id=' . $value["id"] . '" class="mx-1 btn btn-sm btn-outline-primary">查看</a>';
                                            echo '</td>';
                                            echo '</tr>';
                                        }
                                    }
