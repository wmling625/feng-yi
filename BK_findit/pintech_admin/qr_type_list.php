...
$result_arr = array();
$query = "SELECT * FROM qr_type WHERE " . $filter_sql_str . " ORDER BY orders ASC, " . $date_type . " DESC";
...
<div class="wrapper">
    <?php
    echo "<input type='hidden' name='del_sql' value='" . aes_encrypt("DELETE FROM qr_type WHERE find_in_set(qr_type_id, '?1') >0") . "'/>";
    echo "<input type='hidden' name='orders_sql' value='" . aes_encrypt("UPDATE qr_type SET orders = '?1' WHERE find_in_set(qr_type_id, '?2') >0") . "'/>";
    ?>
    <!-- Preloader -->
                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="card-tools">
                                    <button type="button" class="btn btn-sm btn-primary" data-toggle="modal"
                                            data-bs-toggle="tooltip" title="推播給擁有該標籤QRCode的所有會員"
                                            id="lineNotify">
                                        推播訊息
                                    </button>
                                    <a href="qr_type_mang.php?model=add" type="button"
                            ...
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-3 col-sm-6 mt-2">
                                        <input type="text" class="form-control" data-bs-toggle="tooltip" title="標題"
                                               name="keyword"
                                               placeholder="請輸入標題"
...
//                                        $photo_small1 = ($value['file0'] == "") ? "nophoto.png" : $value['file0'];
                                        ...
                                        echo '<input type="checkbox" id="' . $value['qr_type_id'] . '" name="box_list" value="' . $value['qr_type_id'] . '">';
                                        echo '<label for="' . $value['qr_type_id'] . '">';
                                        echo '</label>';
                                        echo '</div>';
                                        echo '</td>';
                                        ...
                                        echo '<a href="qr_type_mang.php?model=update&qr_type_id=' . $value["qr_type_id"] . '" class="mx-1 btn btn-sm btn-outline-primary">查看</a>';
                                        echo '</td>';
                                        echo '<td>';
                                        if ($value['orders'] !== "-1") {
                                            echo '<a href="qrcode_mang.php?model=add&qr_type_id=' . $value["qr_type_id"] . '" class="mx-1 btn btn-sm btn-outline-dark">新增</a>';
                                            echo '<a href="qrcode_list.php?types=' . $value["qr_type_id"] . '" class="mx-1 btn btn-sm btn-outline-dark">查看</a>';
                                        }
                                        echo '</td>';
                                        echo '<td>';
                                        if ($value['orders'] !== "-1") {
                                            echo '<a href="advertisement_list.php?qr=' . $value["qr_type_id"] . '" class="mx-1 btn btn-sm btn-outline-dark">查看</a>';
                                        }
                                        echo '</td>';
                ...
                <div class="modal-content">
                    ...
                    <form method="post" id="form" enctype="multipart/form-data"
                        ...
                        <div class="modal-footer justify-content-between">
                            ...
                            <button type="button" class="btn btn-primary" id="lineNotifyConfirm" name="post">送出</button>
                        </div>
                    </form>
                </div>
                <!-- /.modal-content -->
