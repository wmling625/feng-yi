...
//
	...
	// SMTP 驗證的使用者資訊
	$mail->Host = "mail.nchu.edu.tw"; //此處請填寫您的郵件伺服器位置,通常是mail.網址。如果您MX指到外地，那這邊填入www.XXX.com 即可
	...
	// SMTP 驗證的使用者資訊
	$mail->Username = "amy@mail.nchu.edu.tw";  // 此處為驗証電子郵件帳號,就是您在ServerZoo主機上新增的電子郵件帳號，＠後面請務必一定要打。
	$mail->Password = "sxy-3375";  //此處為上方電子郵件帳號的密碼 (一定要正確不然會無法寄出)

	// 信件內容設定  
	$mail->From = "amy@mail.nchu.edu.tw"; //此處為寄出後收件者顯示寄件者的電子郵件 (請設成與上方驗証電子郵件一樣的位址)
	$mail->FromName = "amy@mail.nchu.edu.tw"; //這裡不能是中文～此處為寄出後收件者顯示寄件者的名稱
	
    $mail->AddReplyTo('amy@mail.nchu.edu.tw', 'amy@mail.nchu.edu.tw');
	...
	$mail->AddAddress("amy@mail.nchu.edu.tw");
