...
$(function () {
    ...
    if ((location.pathname).indexOf("index") >= 0 || location.pathname.substr(-1) == "/") {
    ...
    if ((location.pathname).indexOf("login") >= 0) {
        ...
        $('#mobile').blur(function () {
            $(".progress-btn").removeAttr("disabled").removeClass('wait').html('認證<span></span>');
            $("#mobileValidation").removeClass("hide").find('[name=code]').attr('req', 'Y');
            $("#mobile").closest(".warning-box").find(".alert-text-backend").text('').removeClass("show text-primary text-success");
        })
        ...
        $(".progress-btn").click(function () {
            ...
            if (sum_arr.length > 0) {
                ...
                event_arr['success'] = function (data) {
                    if (data.state == "-1") {
                        $("#mobile").closest(".warning-box").find(".alert-text-backend").text(data.message).addClass("show text-primary");

                    } else if (data.state == "1") {
                        // 簡訊已發送
                        $("#mobile").closest(".warning-box").find(".alert-text-backend").text(data.message).addClass("show text-success");
                        ...
                        $("#mobileValidation").addClass("hide").find('[name=code]').removeAttr('req');
                    }
                }
                ajax_pub_adv("api/sms_notify.php", {
                    "mobile": mobile,
                    "userId": userId,
    ...
    if ((location.pathname).indexOf("fill_in") >= 0) {
        ...
        $('#mobile').blur(function () {
            if ($(this).val() !== "") {
                $(".progress-btn").removeAttr("disabled").removeClass('disabled wait').html('認證<span></span>');
            }
            $("#mobileValidation").removeClass("hide").find('[name=code]').attr('req', 'Y');
            $("#mobile").closest(".warning-box").find(".alert-text-backend").text('').removeClass("show text-primary text-success");
        })
        ...
        $(".progress-btn").click(function () {
            ...
            if (sum_arr.length > 0) {
                ...
                event_arr['success'] = function (data) {
                    if (data.state == "-1") {
                        // 有錯誤
                        $("#mobile").closest(".warning-box").find(".alert-text").text(data.message).addClass("show text-primary");
                    } else if (data.state == "1") {
                        // 簡訊已發送
                        $("#mobile").closest(".warning-box").find(".alert-text").text(data.message).addClass("show text-success");
                        ...
                        // 手機已存在會員，無須重新驗證
                        $("#mobile, #sms-input").closest(".warning-box").find(".alert-text").text('');
                ...
                ajax_pub_adv("api/sms_bind.php", {"mobile": mobile}, event_arr, {"async": true})
            }
        ...
        $(document).on("click", "#sms-btn", function () {
            ...
            if (sumarr.length > 0) {
                ...
                event_arr['success'] = function (data) {
                    if (data.state == "-1" || data.state == "1") {
                        $("#sms-input").closest(".warning-box").find(".alert-text").text(data.message).addClass("show text-primary");
                ...
                ajax_pub_adv("api/sms_validate.php", {"mobile": mobile, "code": code}, event_arr, {"async": true})
            }
    ...
    if ((location.pathname).indexOf("code_list.") >= 0) {
        ...
        $(document).on("click", ".del-btn", function () {
            ...
            if (confirm("確認註銷此筆QR CODE \n#" + info + "?") === false) {
                ...
                ajax_pub_adv("api/code_end.php", {"qrcode_id": id}, event_arr, {"async": true})
            }
        })
