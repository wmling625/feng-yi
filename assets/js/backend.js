    ...
    if ((location.pathname).indexOf("admin") >= 0) {
        ...
        if ((location.pathname).indexOf("member_list.php") >= 0) {
            $("[name=member_qr]").click(function () {
                ...
                ajax_pub_adv("../api/get_member_qr.php", {
                    "id": id
            ...
            $("[name=member_qr_big]").click(function () {
                ...
                ajax_pub_adv("../api/get_member_qr_big.php", {
                    "id": id
                }, event_arr, {"async": true})
            });
        ...
        if ((location.pathname).indexOf("member_mang.php") >= 0) {
            ...
            // 縣市區域
            var city_param = [{
                "city_name": "", "region_name": "", "city_default": "", "region_default": ""
            }, {
                "city_name": "", "region_name": "", "city_default": "", "region_default": ""
            }, {
                "city_name": "", "region_name": "", "city_default": "", "region_default": ""
            ...
            $(document).on('change', '[name^=certificate2_]', function (e) {
                // 宣告物件
                var box_arr = {};
                $(this).parents("td:first").find("[name^=certificate2_]").each(function (i, n) {
                    var key = $(this).val();
            ...
            $('input[name=dob]').daterangepicker({
                autoUpdateInput: false, singleDatePicker: true, showDropdowns: true, locale: {
                    format: "YYYY-MM-DD",
                    applyLabel: "確定",
                    cancelLabel: "取消",
                    fromLabel: "開始日期",
            ...
            $('input[name=renew_date]').on('apply.daterangepicker', function (ev, picker) {
                $(this).val(picker.startDate.format('YYYY-MM-DD'));
            });


            // 根據會員身份 制定不同必填
            $('[name=types_option4]').on('change', function () {
                var qu = $('option:selected', this).data('qu');
                var required = {
                    "p0": [], "p1": [14, 15, 16, 18, 19, 20, 21, 22, 23, 24, 25], // 開業會員
                    "p2": [14, 15, 16, 19, 20, 21, 22, 23, 24, 25], // 執業會員
                    "p3": [14, 15, 24, 25], // 待業會員
                    "p4": [21, 24] // 贊助會員
                };
                $('body').find('[data-id]').each(function (index, item) {
                    $(this).removeAttr("req");
        ...
        if ((location.pathname).indexOf("advertisement_mang.php") >= 0) {
            ...
            // 根據廣告類型 制定不同必填
            $('[name=types_option]').on('change', function () {
                var qu = $('option:selected', this).data('qu');
                var required = {
                    "p0": [], "p1": [3], // 影片廣告
                    "p2": [0, 1, 2], // 圖文連結廣告
                        ...
                        if (id === qid) {
                            ...
                            if (id == 1) {
                                if (qu === "p2") {
                                    ...
                                    $('#adviseSize').html('建議尺寸380px*452px<br/>(約95:113的比例)');
                                }
                            }
                        }
                    })
        ...
        if ((location.pathname).indexOf("change_") >= 0) {
            ...
            // 根據辦理項目 制定不同必填
            $('[name=types_option0]').on('change', function () {
                var qu = $('option:selected', this).data('qu');
                var required = {
                    "p1": [1, 3], // 執業異動-執業
                    "p5": [1, 3, 4], // 執業異動-變更
            ...
            $('[name=types_option1]').on('change', function () {
            ...
            $('input[type=checkbox][name^=types_option2_]').on('click', function () {
                var ids = [];
                $.each($('input[type=checkbox][name^=types_option2_]'), function (index, item) {
        ...
        if ((location.pathname).indexOf("qrcode_mang") >= 0) {
            ...
            $('[name=orders]').on('change', function () {
                var numberArea = $('#numberArea');
                var value = $('option:selected', this).val();
                $('[id^=bindArea]').find('input, select').each(function () {
                    $(this).removeAttr('req')
                    $(this).parents('tr:first').find('.required').text('');
                })
                ...
                if (value === "1" || value == "-2") {
                    ...
                    numberArea.find('input:first,select:first').removeAttr('req').val(0);
                    numberArea.find('.required:first').text('');
                } else {
                    numberArea.removeClass('hide');
                    numberArea.find('input:first,select:first').attr('req', 'Y').val(1);
                ...
                if (code == "") {
                    ...
                    event_arr['success'] = function (data) {
                        ...
                        if (parseInt(data.state) < 0) {
                            $('#code-warning').removeClass('text-success').addClass('text-danger')
                        }

                        if (parseInt(data.state) >= 0) {
                            $('#code-warning').removeClass('text-danger').addClass('text-success')
                        }
                    }

                    ajax_pub_adv("../api/qrcode_validate.php", {
                        "qrcode_id": qrcode_id,
                        "code": code
                    }, event_arr, {"async": true})
                }
                    ...
                    memberModal.on('shown.bs.modal', function (e) {
                        ...
                        $("button[id=lineNotifyConfirm]").click(function (event) {
                            ...
                            if (pintech_trim(ans) === "確定推播") {
                                ...
                                $("button[id=lineNotifyConfirm]").append(' <span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span>')
        ...
        if ((location.pathname).indexOf("qrcode_list") >= 0) {
            ...
            $("button[name=save_qr]").click(function (event) {
                ...
                if (orders === "-1") {
                    ...
                    alert("綁定須填寫相關資料");
                    document.location.href = "qrcode_mang.php?model=update&qrcode_id=" + id;
                    return false;
                }
                    ...
                    memberModal.on('shown.bs.modal', function (e) {
                        ...
                        $("button[id=sendToMemberConfirm]").click(function (event) {
                            ...
                            if (pintech_trim(ans) === "確定發送") {
                                ...
                                var cmd = aes_decrypt($("[name=send_sql]").val()).replaceAll('?2', id);
        ...
        if ((location.pathname).indexOf("gallery_photo_mang.php") >= 0) {
            function add_file(id, file) {
                var template = '<li class="media" id="uploadFile' + id + '">\n' + '        <div class="media-body mb-1">\n' + '          <p class="mb-2">\n' + '            <strong>' + file.name + '</strong><br/>Status: <span class="text-muted">Waiting</span>\n' + '          </p>\n' + '          <div class="progress mb-2">\n' + '            <div class="progress-bar progress-bar-striped progress-bar-animated bg-primary" \n' + '              role="progressbar"\n' + '              style="width: 0%" \n' + '              aria-valuenow="0" aria-valuemin="0" aria-valuemax="100">\n' + '            </div>\n' + '          </div>\n' + '          <hr class="mt-1 mb-1" />\n' + '        </div>\n' + '      </li>';
            ...
            function update_file_status(id, status, message) {
                $('#uploadFile' + id).find('span').html(message).prop('class', 'status text-' + status);
            }
            ...
            $('#drag-and-drop-zone').dmUploader({
                url: 'sp_gallery_photo.php', dataType: 'json', fieldName: 'file0', //這裡是自己設定的
                allowedTypes: 'image/*', extraData: {
        ...
        if ((location.pathname).indexOf("advertisement_mang.php") >= 0 || (location.pathname).indexOf("activity_") >= 0) {
            ...
            $('input[name=bird_date]').on('apply.daterangepicker', function (ev, picker) {
                $(this).val(picker.startDate.format('YYYY-MM-DD'));
            });
        }
        ...
        if ((location.pathname).indexOf("_mang.php") >= 0) {
            ...
            $(document).on('change', '[type=checkbox]', function (e) {
                var box_arr = {}; //宣告物件
                $(this).parents("td:first").find("[type=checkbox]:checked").each(function (i, n) {
                    var key = $(this).val();
            ...
            $("button[name=post]").click(function (e) {
                ...
                $(this).prop('disabled', true).append(" <span class=\"spinner-grow spinner-grow-sm mx-1\" role=\"status\" aria-hidden=\"true\"></span>");
                var this_form = Boolean($(this).parents('form')) ? $(this).parents('form') : $('#' + $(this).attr('form'));
                ...
                $("[req]:not([readonly],[disabled]").each(function () {
                    ...
                    if ($(this).attr('type') === 'file') {
                        ...
                        if ($(this).val() === "") {
                            ...
                            if ((location.pathname).indexOf("member_mang.php") >= 0) {
                                if ($(this).attr('name') === 'account' && !ValidateID($(this).val())) {
                                    sum_arr.push("身分證字號格式錯誤");
                                    $(this).addClass('is-invalid');
                                }
                            }

                            if ((location.pathname).indexOf("qrcode_mang.php") >= 0 && model === "add") {
                                if ($(this).attr('type') === 'number') {
                                    var min = parseInt($(this).attr('min'));
                                    var max = parseInt($(this).attr('max'));
                                    if (parseInt($(this).val()) >= min && parseInt($(this).val()) <= max) {
                                    } else {
                                        sum_arr.push("「" + title + "」請介於 " + min + " 至 " + max);
                                        $(this).addClass('is-invalid');
                                    }
                                }
                            }
        ...
        if ((location.pathname).indexOf("_list.php") >= 0) {
            ...
            $("button[name=box_del]").click(function (event) {
                ...
                if (id != "") {
                    ...
                    if (pintech_trim(ans) === "確定刪除") {
                        ...
                        var cmd = aes_decrypt($("[name=del_sql]").val()).replaceAll('?1', id);
                        // 再加密一次，送進sp_command.php
                        cmd = aes_encrypt(cmd);
                        var temp = gettoken_value();
                        var value = temp.value;
            ...
            $("button[name=save]").click(function (event) {
                ...
                var id = $(this).parents("tr").find("[name=box_list]").val();
                // 找到 update sql語法，並將orders(?1)、id(?2)帶入參數，再加密
                var cmd = aes_decrypt($("[name=orders_sql]").val()).replace('?1', orders);
                cmd = cmd.replace('?2', id);
                cmd = aes_encrypt(cmd);
            ...
            $("[name=zip_button]").click(function (event) {
                ...
                var value = temp.value;
                var token = temp.token;
                window.open(sp + "?cmd=" + cmd + "&value=" + value + "&token=" + token + "", '_blank');
                // document.location.href = sp + "?cmd=" + cmd + "&value=" + value + "&token=" + token + "";
            })
